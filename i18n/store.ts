...
const LANG_LOCAL_KEY = '__age_lang';
...
export class LangStore {
    ...
    async import(lang?: Langs) {
        ...
        var r = localStorage.getItem(LANG_LOCAL_KEY)
        if (typeof r == 'string') lang = r as Langs;
        if (typeof lang != 'string') {
            lang = window.ageConfig.lang;
        }
        this.lang = lang;
        await this.load();
        this.isLoaded = true;
    ...
    async load() {
        var rc = localStorage.getItem(`__shy_lang_data_${this.lang}`) as any;
        if (typeof rc == 'string') rc = JSON.parse(rc);
        var rs: Record<string, any>;
        if (rc) {
            if (false && rc.version == window.ageConfig.version && window.ageConfig.isPro) {
                rs = rc.langs;
            }
        }
        if (!rs) {
            ...
            localStorage.setItem(`__shy_lang_data_${this.lang}`, JSON.stringify({ version: window.ageConfig.version, langs: rs }));
    ...
    t(key: string, content?: string | Record<string, any>, checkExists?: boolean) {
        var d;
        if (typeof window.ageConfig.isUS) {
