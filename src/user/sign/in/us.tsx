    ...
    renderInputEmail() {
        ...
        async function checkInputEmail(button: Button) {
            ...
            try {
                ...
                var r = await authSock.get('/account/is/sign', { account: local.email });
                if (r?.ok) {
                    if (r.data.sign) {
                        local.failMsg = '';
                        local.step = 'login';
    ...
    renderLogin() {
        ...
        async function login(button: Button) {
            try {
                ...
                var r = await authSock.post('/account/login', {
                    account: local.email,
        ...
        async function genCode() {
            if (local.expireCount == -1) {
                ...
                var result = await authSock.post('/account/send/verify/code', { account: local.email });
                if (result.ok) {
