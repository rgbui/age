    ...
    move = (event: MouseEvent) => {
        if (scope.isDown == true) {
            ...
            if (scope.isMove == true) {
                ...
                if (options.isCross) {
                    var cp = dom(event.target as HTMLElement).closest(x => typeof (x as HTMLElement).seer_drop_move == 'function') as HTMLElement;
                    if (cp) {
                        if (!crossPanels.some(s => s === cp)) crossPanels.push(cp);
                        cp.seer_drop_move(crossData.type, crossData.data,event);
                    }
                }
            }
            else {
             ...
             */
                ...
                if (options.isCross) {
                    try {
                        var cp = dom(event.target as HTMLElement).closest(x => typeof (x as HTMLElement).seer_drop_move == 'function') as HTMLElement;
                        if (cp) {
                            cp.seer_drop_over(crossData.type, crossData.data,event);
                        }
                        if (crossPanels.length > 0) {
                            for (let i = 0; i < crossPanels.length; i++) {
                                crossPanels[i].seer_end(event)
                            }
                        }
                    }
                    catch (ex) {
