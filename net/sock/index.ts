...
export class Sock {
    ...
    private get remote() {
        if (this._remote) return this._remote;
            ...
            if (window.ageConfig.isDev) this._remote.defaults.timeout = 1000 * 60 * 10;
            else if (window.ageConfig.isBeta || window.ageConfig.isPro) {
 ...
 * 主服务（主要用来服务于整个系统的的业务）
 */
export var authSock = new Sock(SockType.auth);
/**
 * 主要是用来上传文件
 */
export var ageSock = new Sock(SockType.age);
